...
declare -a trig_fn=( "log" "sin" "cos" "tan" "ctg" "acos" "asin" "atan" "sqrt" )
...
for (( lines=1; lines<=$default; lines++)); do
    ...
    for (( j=0; j<$memb_of_expr; j++ )); do
        ...
        declare -a variables=( "$rand_float" "$rand_float2" )
    ...
    echo "$done_expr" | sed 's/ $//' | sed 's/,$//' >> eval_fn.txt
    echo "$done_expr" | tr -d , | tr -d \" | tr -d ' ' >> eval_fn_str.txt
done
